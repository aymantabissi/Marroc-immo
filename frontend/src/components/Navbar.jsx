...
export default function Navbar({ pageName = "Tableau de Bord" }) {
  ...
  const [user, setUser] = useState({ name: "Utilisateur", role: "client" });
  ...
  return (
    <nav className="bg-gray-900 text-white px-6 py-4 flex justify-between items-center shadow-md">
      ...
      {/* Center: Navigation Links (Admin vs Client) */}
      <div className="flex-1 flex justify-center gap-6">
        {user.role === "client" ? (
          <>
            <Link to="/client/home" className="hover:text-blue-400 transition-colors">Accueil</Link>
            <Link to="/client/acheter" className="hover:text-blue-400 transition-colors">Acheter</Link>
            <Link to="/client/louer" className="hover:text-blue-400 transition-colors">Louer</Link>
            <Link to="/client/contact" className="hover:text-blue-400 transition-colors">Contactez-nous</Link>
          </>
        ) : (
